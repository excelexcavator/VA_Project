...
#----------------------------------- adjusting paramters ----------------------------------------#		
file_in = open('./configs/test_config.yaml')
f = yaml.load(file_in)
file_in.close()
file_out = open('./configs/test_config.yaml', 'w')
    ...
    for i, vfeat in enumerate(video_loader):
        ...
        for j, afeat in enumerate(audio_loader):
            for k in np.arange(bz):
                ...
                vfeat0, afeat0 = model(vfeat_var, afeat_var)
                cur_sim = F.pairwise_distance(vfeat0, afeat0)
                if k == 0:
        ...
        if i == 0:
            sim_mat1 = slice_sim.clone()
        else:
            sim_mat1 = torch.cat((sim_mat1, slice_sim.clone()), 1)
 
    for i, afeat in enumerate(audio_loader):
        bz = afeat.size()[0]
        # sample_num += bz
        for j, vfeat in enumerate(video_loader):
            for k in np.arange(bz):
                cur_afeat = afeat[k].clone()
                cur_afeats = cur_afeat.repeat(bz, 1, 1)
                afeat_var = Variable(cur_afeats, volatile=True)
            ...
            if j == 0:
                ...
                slice_sim = torch.cat((slice_sim, bz_sim.clone()), 0)
        if i == 0:
            sim_mat2 = slice_sim.clone()
        else:
            sim_mat2 = torch.cat((sim_mat2, slice_sim.clone()), 1)
	sim_mat = (sim_mat1 + sim_mat2)/2;
